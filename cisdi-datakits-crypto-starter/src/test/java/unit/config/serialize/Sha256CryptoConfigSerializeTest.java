...
public class Sha256CryptoConfigSerializeTest extends SerializeBaseTest {

    @ParameterizedTest
    @ValueSource(booleans = { true, false })
    void success_sha256_random_salt_deserialize(boolean isRandomSalt) {
        assertDoesNotThrow(() -> {
            CryptoConfig cryptoConfig = new Sha256CryptoConfig()
            ...
            if (isRandomSalt) {
                assertThat(json).isEqualTo("{\"cryptoType\":\"SHA256\",\"salt\":\"123\",\"saltSize\":6,\"randomSalt\":true}");
            } else {
                assertThat(json).isEqualTo("{\"cryptoType\":\"SHA256\",\"salt\":\"123\",\"saltSize\":6,\"randomSalt\":false}");
            }
    ...
    @Test
    void success_serialize_sha256_fix_salt() {
        assertDoesNotThrow(() -> {
            CryptoConfig cryptoConfig = new Sha256CryptoConfig()
                    .setSalt("123");
            System.out.println("原始配置：");
            ...
            System.out.println("原始配置序列化json：");
            System.out.println(json);
            assertThat(json).isEqualTo("{\"cryptoType\":\"SHA256\",\"salt\":\"123\",\"saltSize\":8,\"randomSalt\":false}");
