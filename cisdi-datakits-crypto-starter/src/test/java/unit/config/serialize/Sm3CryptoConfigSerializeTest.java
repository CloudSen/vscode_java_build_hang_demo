...
public class Sm3CryptoConfigSerializeTest extends SerializeBaseTest {

    @ParameterizedTest
    @ValueSource(booleans = { true, false })
    void success_sm3_random_salt_deserialize(boolean isRandomSalt) {
        assertDoesNotThrow(() -> {
            CryptoConfig cryptoConfig = new Sm3CryptoConfig()
            ...
            if (isRandomSalt) {
                assertThat(json).isEqualTo("{\"cryptoType\":\"SM3\",\"salt\":\"123\",\"saltSize\":6,\"randomSalt\":true}");
            } else {
                assertThat(json).isEqualTo("{\"cryptoType\":\"SM3\",\"salt\":\"123\",\"saltSize\":6,\"randomSalt\":false}");
            }
    ...
    @Test
    void success_serialize_sm3_fix_salt() {
        assertDoesNotThrow(() -> {
            CryptoConfig cryptoConfig = new Sm3CryptoConfig()
                    .setSalt("123");
            System.out.println("原始配置：");
            ...
            System.out.println("原始配置序列化json：");
            System.out.println(json);
            assertThat(json).isEqualTo("{\"cryptoType\":\"SM3\",\"salt\":\"123\",\"saltSize\":8,\"randomSalt\":false}");
